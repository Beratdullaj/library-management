<template>
  <VueDropzone
    ...
    @vdropzone-removed-file="removeFileNews"
...
export default {
  ...
  data() {
    return {
      dropzoneOptions: {
        url: `http://localhost:4000/news/${this.newsId}/uploadFile`,
        method: "put",
        thumbnailWidth: 300,
        maxFilesize: 2,
        addRemoveLinks: true,
  ...
  methods: {
      ...
      removeFileNews(file){
        apiRequest.removeFileNews(this.newsId, file);
      },
    ...
    async handleMount() {
      ...
      const documentFiles = await apiRequest.getFilesDetailsNews(this.initialFiles);
